...
const dropzone = useDropzone({
    ...
    disabled: files.length >= 5, // Disable drop zone when 5 files are uploaded
});
...
const Input = ({ dropzone, files }: InputProps & { files: File[] }) => {
    ...
    return (
        ...
        style={{
            ...
            cursor: 'pointer',
            opacity: files.length >= 5 ? 0.5 : 1, // Reduce opacity when 5 files are uploaded
        }}
        >
        <input {...getInputProps()} className="hidden" />
        <AddAPhotoIcon sx={{ fontSize: 75, color: isDragActive ? 'blue' : 'gray' }} />
        {isDragActive ? (
